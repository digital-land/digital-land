...
* Our research showed a number of fields didn't add any information, were rarely used by publishers, or useful to users, so we have [deprecated](https://en.wikipedia.org/wiki/Deprecation#Software_deprecation) them.
* There were 6 separate text fields which weren't often blank or misused, so we have combined them into a single "Notes" field.
